...
- [2. Metamodel Anchor Points](#2-metamodel-anchor-points)
- [3. Semantic Overlap Summary](#3-semantic-overlap-summary)
- [4. Correspondence Rules in Detail](#4-correspondence-rules-in-detail)
- [5. OCL Invariant Summary](#5-ocl-invariant-summary)
- [6. Consistency Preservation Rules](#6-consistency-preservation-rules)
...
|---|---|---|
| `Component` | `ComponentsModel.components` | has `name`, `runnables[]`, `labels[]`, `ports[]` |
| `Task` / `ISR` | `SWModel.tasks` / `.isrs` | key refs: `stimuli[]`, `activityGraph` |
| `Runnable` | `SWModel.runnables` | execution described by `activityGraph.items` |
...
The ten correspondences below cover every row in Table 5.1. Each is expanded fully in [Section 4](#4-correspondence-rules-in-detail).
...
> Every `Method` inside the `Module` must have `returnType = null`, and every such `Method` must have an empty `parameters` list.
...
- **OCL filter:** none at the class level
...
> A `Runnable` models a void procedure with no formal parameters at the interface level. `RunnableParameter` instances in AMALTHEA do **not** propagate to ASEM `Method` parameters — the OCL constraint explicitly forbids parameters on the `Method` side.
...
> A `BaseTypeDefinition` of exactly 1 bit maps to ASEM `BooleanType`. ASCET calls this `Log` (logical / boolean); single-bit representation.
...
> [!IMPORTANT]
> Size 32 appears in both Rule 8 and Rule 9. Tie-breaker: if the alias contains `float` or `double` → Rule 9 (`ContinuousType`); otherwise → Rule 8 (`DiscreteType`). E.g. `sint32`/`uint32` → `DiscreteType`; `float32` → `ContinuousType`.
...
> Maps to `ContinuousType` in `asem.primitivetypes` — covers floating-point / real-valued types. Overlap with Rule 8 at size=32 resolved by alias — see note in Rule 8.
